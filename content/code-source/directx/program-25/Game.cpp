...
bool MyDX3DGame::InitializeDirectX() {
	...
	if (!LoadOBJ("vis-strange-map.obj", mapVertices)) return false;
...
void MyDX3DGame::CreateDoor(XMFLOAT3 position, float currentRotation, float targetRotation, bool isOpen)
{
	...
	myDoor.collisionBox = {
		{ position.x        , position.y         , position.z - thickness },
...
void MyDX3DGame::Update() {
	...
	for (auto& door : m_Doors) {
		...
		if (fPressed && XMVectorGetX(XMVector3Length(pPos - dPos)) < 2.5f) {
