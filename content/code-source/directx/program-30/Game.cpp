...
void MyDX3DGame::Update() {
	...
	if (m_IsGrounded) {
		...
		if (GetAsyncKeyState(VK_SPACE) & 0x8000) {
			...
			m_IsGrounded = false;
			PlaySoundFX(m_pSourceVoiceJump, m_sndJump); // <-- TOCAR PULO
		}
	}
	else {
	...
	// Definimos 5 pontos: Centro + 4 direções (Norte, Sul, Leste, Oeste)
	// Usamos um raio um pouco menor (80%) que o charRadius para evitar prender em quinas finas
	float checkRadius = charRadius * 0.8f;
	XMFLOAT3 testPoints[5] = {
		{ m_PlayerPos.x, m_PlayerPos.y + 1.0f, m_PlayerPos.z }, // Centro
		{ m_PlayerPos.x + checkRadius, m_PlayerPos.y + 1.0f, m_PlayerPos.z },
		{ m_PlayerPos.x - checkRadius, m_PlayerPos.y + 1.0f, m_PlayerPos.z },
		{ m_PlayerPos.x, m_PlayerPos.y + 1.0f, m_PlayerPos.z + checkRadius },
		{ m_PlayerPos.x, m_PlayerPos.y + 1.0f, m_PlayerPos.z - checkRadius }
	};

	for (int i = 0; i < 5; i++) {
		XMVECTOR rayOrigin = XMLoadFloat3(&testPoints[i]);
		XMVECTOR rayDir = XMVectorSet(0, -1.0f, 0, 0);

		for (const auto& tri : m_FloorTriangles) {
			float dist = 0.0f;
			if (DirectX::TriangleTests::Intersects(rayOrigin, rayDir, XMLoadFloat3(&tri.v0), XMLoadFloat3(&tri.v1), XMLoadFloat3(&tri.v2), dist)) {
				if (dist < closestDist) {
					closestDist = dist;
					hit = true;
				}
	...
	if (hit) {
		float floorY = (m_PlayerPos.y + 1.0f) - closestDist;
...
MyDX3DGame::~MyDX3DGame()
{
	...
	// --- Adicione no início do seu ~MyDX3DGame() ---
	...
	// Remove qualquer referência ativa da GPU para evitar travamento de contagem (ref count)
	if (m_pDevCon) {
		m_pDevCon->OMSetRenderTargets(0, nullptr, nullptr);
	...
	// 2. RECURSOS DE GEOMETRIA (Criados no Initialize)
