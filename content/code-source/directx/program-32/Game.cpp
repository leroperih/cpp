...
bool MyDX3DGame::InitializeDirect2D() {
	...
	// Desvincula o alvo antigo se ele existir para evitar vazamento de memória
	...
	// Cria o bitmap mapeando a superfície do DX3D
	ComPtr<ID2D1Bitmap1> pD2dTargetBitmap;
	hr = m_pD2DContext->CreateBitmapFromDxgiSurface(pBackBufferSurface.Get(), &bitmapProperties, &pD2dTargetBitmap);
	if (FAILED(hr)) return false;

	// Define o bitmap criado como o alvo de renderização atual do Direct2D
	m_pD2DContext->SetTarget(pD2dTargetBitmap.Get());
	return true;
}
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				// --- 3. OPÇÃO 2: SAIR DO JOGO ---
					...
					const wchar_t menu_text_opt_02[] = L"Menu";
				...
				// --- 3. OPÇÃO 2: SAIR DO JOGO ---
					D2D1_RECT_F opt3Rect = D2D1::RectF(0.0f, (float)m_WndHeight * 0.66f, (float)m_WndWidth, (float)m_WndHeight * 0.71f);
					// Cria o retângulo menor centralizado para o fundo da Opção 2
					m_menuOpt3Rect = D2D1::RectF(leftPos, opt3Rect.top, rightPos, opt3Rect.bottom);
					if (m_SelectedOption == 2) {
						m_pScreenBrush->SetColor(D2D1::ColorF(D2D1::ColorF::White, 0.15f));
						m_pD2DContext->FillRectangle(&m_menuOpt3Rect, m_pScreenBrush.Get());
						m_pScreenBrush->SetColor(D2D1::ColorF(D2D1::ColorF::Yellow));
					}
					else {
						m_pScreenBrush->SetColor(D2D1::ColorF(D2D1::ColorF::Black, 0.3f));
					...
					const wchar_t menu_text_opt_03[] = L"Sair do Jogo";
					m_pD2DContext->DrawTextW(menu_text_opt_03, (sizeof(menu_text_opt_03) / sizeof(const wchar_t)), m_pTextFormat.Get(), opt3Rect, m_pScreenBrush.Get());
	...
	switch (uMsg)
	{
		...
		case WM_LBUTTONDOWN: // Clicou na tela
				...
				if (m_isPaused)
				{
					...
					{
						MessageBoxA(NULL, "O menu ainda nao esta pronto!", "ERRO", MB_ICONERROR);
					}
					else if (mouseX >= m_menuOpt3Rect.left && mouseX <= m_menuOpt3Rect.right && mouseY >= m_menuOpt3Rect.top && mouseY <= m_menuOpt3Rect.bottom)
					{
						PostQuitMessage(0);
					}
				}
