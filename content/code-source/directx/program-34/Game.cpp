...
bool MyDX3DGame::InitializeDirectX() {
	...
	// 6.1 Carregar Mapa (Usa 3 parâmetros: precisa de colisões e txt)
	std::vector<VertexOBJ> mapVertices;
	if (!LoadOBJ("mapa-detalhado.obj", "mapa-transforms.txt", mapVertices))
	{
		MessageBoxA(m_hWnd, "ERRO: A funcao LoadOBJ nao conseguiu carregar o modelo de mapa!", "Erro ao Carregar Modelo 3D", MB_ICONERROR);
		return false;
	}
...
void MyDX3DGame::Update() {
	...
	m_MoveSpeed = (m_IsRunning) ? 12.0f : 6.0f;
	...
	auto GetWallCollisionNormal = [&](const AABB& pBox, XMVECTOR& outNormal) -> bool {
		...
		XMStoreFloat3(&playerBox.Center, pCenter);
		XMStoreFloat3(&playerBox.Extents, pExtents);

		// 1. Varre as OBBs de Paredes do cenário
		for (const auto& wall : m_WallOBBs) {
			...
			if (wallOriented.Intersects(playerBox)) {
				// Calcula a direção do impacto entre os centros
				XMVECTOR wCenter = XMLoadFloat3(&wallOriented.Center);
				XMVECTOR collisionDir = pCenter - wCenter;

				// Extrai os eixos direcionais reais da OBB rotacionada
				...
				// Projeta a direção nos eixos locais para descobrir qual face foi atingida
				...
				// Escolhe a face com base na maior proximidade/projeção absoluta
				if (fabs(dotX) > fabs(dotY) && fabs(dotX) > fabs(dotZ)) {
					outNormal = (dotX > 0.0f) ? obbX : -obbX;
				}
				else if (fabs(dotY) > fabs(dotX) && fabs(dotY) > fabs(dotZ)) {
					outNormal = (dotY > 0.0f) ? obbY : -obbY;
				}
				else {
					outNormal = (dotZ > 0.0f) ? obbZ : -obbZ;
				}

				// Zera o componente Y para evitar que colisões horizontais empurrem o jogador para o alto
		...
		// 2. Varre as portas lógicas do mapa (Mantém tratamento AABB padrão)
		for (const auto& door : m_Doors) {
			...
			if (doorDxBox.Intersects(playerBox)) {
				// Retorna uma normal aproximada simples baseada nos centros para portas normais
				XMVECTOR dCenter = XMLoadFloat3(&doorDxBox.Center);
				XMVECTOR dir = XMVectorSetY(pCenter - dCenter, 0.0f);
				outNormal = XMVector3Normalize(dir);
				return true;
			}
		}
		return false;
	...
	// --- PROCESSAMENTO DO MOVIMENTO COM DEVOLUÇÃO TANGENCIAL ---

	// Aplica o passo inicial no Eixo X
	...
	if (GetWallCollisionNormal(playerBoxX, normalX)) {
		// Desfaz o movimento puro que causou a penetração
		m_PlayerPos.x -= velocity.x;

		// Remove a força aplicada diretamente contra a parede, gerando o vetor de deslize
		...
		if (XMVectorGetX(penetration) < 0.0f) {
			...
			// Aplica apenas o desvio horizontal resultante no Eixo X
			m_PlayerPos.x += XMVectorGetX(slideVel);
		}
	}

	// Aplica o passo inicial no Eixo Z
	...
	XMVECTOR normalZ;
	if (GetWallCollisionNormal(playerBoxZ, normalZ)) {
		// Desfaz o movimento puro que causou a penetração
		m_PlayerPos.z -= velocity.z;

		// Remove a força aplicada diretamente contra a parede no plano Z
		XMVECTOR currentVel = XMLoadFloat3(&velocity);
		XMVECTOR penetration = XMVector3Dot(currentVel, normalZ);
		if (XMVectorGetX(penetration) < 0.0f) {
			XMVECTOR slideVel = currentVel - (penetration * normalZ);
			// Aplica apenas o desvio horizontal resultante no Eixo Z
...
void MyDX3DGame::Render() {
	...
	if (m_isSettings_screen && m_pD2DContext)
	{
		...
		m_SettingsButtons.push_back({ 0 ,  0.5f , leftPos , rightPos , m_menu_settings_Opt1Rect , L"Voltar" });
		m_SettingsButtons.push_back({ 1 , 0.58f , leftPos , rightPos , m_menu_settings_Opt2Rect , L"Relatorio" });
		m_SettingsButtons.push_back({ 2 , 0.66f , leftPos , rightPos , m_menu_settings_Opt3Rect , L"Debug Boxes" });
...
void MyDX3DGame::ShowMenu(std::vector<Button>& buttons, MenuTitle title)
{
	...
	// PASSO CRUCIAL C: Define o alvo que já foi criado no OnResize (Sem alocações!)
	m_pD2DContext->SetTarget(m_pD2DBackBufferTarget.Get());

	// PASSO CRUCIAL D: Desvincular Shaders do DirectX 11 temporariamente
	...
	m_pD2DContext->DrawTextW(title.text, titleLength, m_pTextFormat.Get(), title.rect, m_pScreenBrush.Get());
	...
	for (auto& button : buttons)
	{
		...
		// Atualiza a hitbox real do botão dentro do vetor que veio por referência
		button.hitbox = D2D1::RectF(button.left, topPos, button.right, bottomPos);

		// CORREÇÃO DOS RECTS: Atualiza os retângulos globais da classe para o clique do mouse funcionar
		if (m_isPause_screen)
		{
		...
		// Estilização visual (Fundo dos botões)
		if (m_SelectedOption == button.id)
		{
			...
			m_pScreenBrush->SetColor(D2D1::ColorF(D2D1::ColorF::Yellow)); // Texto Amarelo para seleção
		}
		else
		{
			m_pScreenBrush->SetColor(D2D1::ColorF(D2D1::ColorF::Black, 0.3f)); // Levemente escuro e transparente
			m_pD2DContext->FillRectangle(&button.hitbox, m_pScreenBrush.Get());
			m_pScreenBrush->SetColor(D2D1::ColorF(D2D1::ColorF::White)); // Texto branco padrão
		}

		// Desenha o texto da opção atual
		UINT32 textLength = (UINT32)wcslen(button.text);
		m_pD2DContext->DrawTextW(button.text, textLength, m_pTextFormat.Get(), button.hitbox, m_pScreenBrush.Get());
	}

	m_pD2DContext->EndDraw();

	// Libera o alvo para o próximo frame
	m_pD2DContext->SetTarget(nullptr);
}



void MyDX3DGame::ButtonActions(int which_menu, int opt)
{
	if (which_menu == 1) // MENU DE PAUSE
	{
		if (opt == 0) // Continuar Jogo
		{
		...
		else if (opt == 1) // Ir para Configurações
		{
			m_isPause_screen = false;
			m_isSettings_screen = true;
			m_SelectedOption = 0; // CORRIGIDO: Inicia o menu de configurações na primeira opção
		}
		else if (opt == 2) // Sair do Jogo
		{
			PostQuitMessage(0);
		}
	}
	else if (which_menu == 2) // MENU DE CONFIGURAÇÕES
	{
		if (opt == 0) // Voltar para o Pause
		{
			m_isPause_screen = true;
			m_isSettings_screen = false;
			m_SelectedOption = 0; // CORRIGIDO: Reseta para a primeira opção ao voltar para o pause
		}
		else if (opt == 1) // Mais Informações
		{
			MoreInformation(1);
		}
		else if (opt == 2) // Alternar Debug Screen
		{
...
void MyDX3DGame::MoreInformation(int opt)
{
...
LRESULT MyDX3DGame::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_ACTIVATE:
	{
		...
		if (activationState == WA_INACTIVE)
		{
			...
			// Força o contador do Windows a ficar visível (>= 0)
	...
	case WM_LBUTTONDOWN:
	{
		...
		if (m_isPause_screen)
		{
			if (IsHovered(m_menu_pause_Opt1Rect, mouseX, mouseY)) ButtonActions(1, 0);
			else if (IsHovered(m_menu_pause_Opt2Rect, mouseX, mouseY)) ButtonActions(1, 1);
			else if (IsHovered(m_menu_pause_Opt3Rect, mouseX, mouseY)) ButtonActions(1, 2);
		}
		else if (m_isSettings_screen)
		{
			if (IsHovered(m_menu_settings_Opt1Rect, mouseX, mouseY)) ButtonActions(2, 0);
			else if (IsHovered(m_menu_settings_Opt2Rect, mouseX, mouseY)) ButtonActions(2, 1);
			else if (IsHovered(m_menu_settings_Opt3Rect, mouseX, mouseY)) ButtonActions(2, 2);
		}
		// CORRIGIDO: Só esconde se ele NÃO estivesse capturado antes (evita bug de ShowCursor acumulado)
		else if (!m_IsMouseCaptured)
		{
			m_IsMouseCaptured = true;
			while (ShowCursor(FALSE) >= 0); // Garante que escondeu de primeira
	...
	case WM_KEYDOWN:
		if (wParam == VK_ESCAPE)
		{
			if (m_isSettings_screen)
			{
				m_isSettings_screen = false;
				m_isPause_screen = true;
				m_SelectedOption = 0; // Reseta o índice para o menu anterior
			}
			else
			{
				m_isPause_screen = !m_isPause_screen;
		...
		// CORRIGIDO: Processamento do menu de Pause (Apenas se a tela de settings NÃO estiver aberta)
		if (m_isPause_screen && !m_isSettings_screen)
		{
			if (wParam == VK_UP || wParam == 'W')
				m_SelectedOption = (m_SelectedOption - 1 + m_pause_MaxOptions) % m_pause_MaxOptions;

			if (wParam == VK_DOWN || wParam == 'S')
				m_SelectedOption = (m_SelectedOption + 1) % m_pause_MaxOptions;

			if (wParam == VK_RETURN)
				ButtonActions(1, m_SelectedOption); // Otimizado: passa a opção direta

			return 0;
		}
		// CORRIGIDO: Agora funciona perfeitamente de forma isolada
		else if (m_isSettings_screen)
		{
			if (wParam == VK_UP || wParam == 'W')
				m_SelectedOption = (m_SelectedOption - 1 + m_settings_MaxOptions) % m_settings_MaxOptions;

			if (wParam == VK_DOWN || wParam == 'S')
				m_SelectedOption = (m_SelectedOption + 1) % m_settings_MaxOptions;

			if (wParam == VK_RETURN)
				ButtonActions(2, m_SelectedOption);
	...
	case WM_INPUT:
	{
		...
		{
			// CORRIGIDO: Só processa se for movimento relativo (ignora cliques redundantes)
	...
	// ADICIONADO: Encerra o jogo corretamente ao clicar no "X" da janela
	case WM_DESTROY:
		PostQuitMessage(0);
		return 0;
	}
